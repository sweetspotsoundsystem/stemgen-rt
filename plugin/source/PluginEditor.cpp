...
void AudioPluginAudioProcessorEditor::paint(juce::Graphics& g) {
  ...
  auto readout = area.removeFromBottom(36);
  area.removeFromBottom(12);

#if HAS_LOGO_ASSET
  if (logoImage.isValid()) {
    g.drawImageWithin(logoImage, area.getX(), area.getY(), area.getWidth(),
                      area.getHeight(), juce::RectanglePlacement::centred);
  } else
#endif
  {
    g.setColour(juce::Colours::white);
    g.setFont(juce::FontOptions(24.0f));
    g.drawFittedText("StemgenRT", area, juce::Justification::centred, 1);
  }

  g.setColour(juce::Colours::white);
  g.setFont(juce::FontOptions(12.0f));
  g.drawFittedText(
      "Latency: " + juce::String(processorRef.getLatencyMs(), 2) + " ms (" +
          juce::String(processorRef.getLatencySamples()) + " samples)",
      readout.removeFromTop(18), juce::Justification::centred, 1);
  g.drawFittedText("Fallback: " +
                       juce::String(static_cast<juce::int64>(
                           processorRef.getUnderrunSampleCount())) +
                       " samples",
                   readout, juce::Justification::centred, 1);
}
