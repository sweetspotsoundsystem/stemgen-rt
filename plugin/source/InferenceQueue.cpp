...
void InferenceQueue::inferenceThreadFunc(WorkerCallbacks callbacks) {
  ...
  while (!shouldStop_.load(std::memory_order_acquire)) {
    ...
    while (true) {
      ...
      // Run inference. c214 emits the current input hop, including a valid
      // sequence-zero c214 output immediately after reset.
      ...
      // first so the next successful run starts from all-zero c214 state.
      if (!inferenceOk) {
