...
TEST(ModelOutputSchedulerTest,
     SequenceZeroMapsToFirstHopAndSequenceOneToNextHop) {
  const auto first =
      planModelOutputSchedule(0U, kLatency, kLatency, 4U * kChunkSize);
  ASSERT_EQ(first.action, ModelOutputScheduleAction::kSchedule);
  EXPECT_EQ(first.firstTimelineSample, kLatency);
  EXPECT_EQ(first.scheduleTimelineSample, kLatency);
  EXPECT_EQ(first.sourceOffset, 0U);
  EXPECT_EQ(first.sampleCount, kChunkSize);

  const auto second =
      planModelOutputSchedule(1U, kLatency, kLatency, 4U * kChunkSize);
  ASSERT_EQ(second.action, ModelOutputScheduleAction::kSchedule);
  EXPECT_EQ(second.firstTimelineSample, kLatency + kChunkSize);
  EXPECT_EQ(second.scheduleTimelineSample, kLatency + kChunkSize);
}
