...
TEST(TransportUnderrunE2ETest,
  ...
  // its post-latency samples. While stopped, this is idle pipeline fill.
  ...
  // c236 itself has no graph flush, but the plugin still owes the final
  ...
  EXPECT_GT(maximumTailRetainedStem, 1.0e-3f)
      << "The play-to-stop callback did not drain c236's queued final hop";
...
TEST(TransportUnderrunE2ETest,
     SmallPreparedHostBlocksFailClosedForC236HostContract) {
