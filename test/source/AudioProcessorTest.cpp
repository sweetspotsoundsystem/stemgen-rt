...
TEST_F(AudioProcessorTest, OrtStatusString) {
  ...
  EXPECT_TRUE(
      ...
      status.containsIgnoreCase("requires 44.1 kHz / 256 samples"));
}
...
TEST_F(AudioProcessorTest,
       LargePreparedBlockFailsClosedForC214HostContract) {
...
TEST(ConstantsTest, StatefulStreamingWindowIsFourHops) {
...
TEST(ConstantsTest, C214HistoryShapesMatchThreeStateContract) {
...
TEST(ConstantsTest, PreservedBridgeMathIsSeparateFromCurrentChunkQualification) {
...
TEST_F(AudioQualityTest, StereoImageIsPreserved) {
  ...
  // The c214 graph emits the current hop, while the asynchronous publication
...
TEST_F(BassDiagnosticTest, DiagnoseKickDrum) {
  ...
  // Discontinuities at boundaries cause ~172Hz artifacts that sound like
  // saturation.
  {
