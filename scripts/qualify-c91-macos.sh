#!/bin/bash
# Fail-closed target-Mac machine qualification for the hardened c91 runtime.
# This script never installs, publishes, tags, or promotes a plug-in.

set -euo pipefail

EXPECTED_MODEL_SHA256="52fdc46d015819821dae19ef272b6bc4ccf441a0274d7d9c8bb44af50eafef8c"
EXPECTED_MODEL_BYTES="129088022"
...
EXPECTED_CALLBACK_SAMPLES="512"
EXPECTED_PDC_SAMPLES="1024"
MAX_POST_WARMUP_RSS_GROWTH_KIB="131072"
MAX_END_RSS_GROWTH_KIB="8192"

usage() {
    cat <<'EOF'
Usage: ./scripts/qualify-c91-macos.sh NEW_EVIDENCE_DIRECTORY
...
if [[ -f HANDOFF_CONTENTS.sha256 && ! -L HANDOFF_CONTENTS.sha256 ]]; then
    ...
    printf '%s  c91-hardened-23ms-source.tar.gz\n' "$HANDOFF_ARCHIVE_SHA256" \
...
2. Fully restart the DAW. At 44.1 kHz / 512 samples, confirm the loaded plug-in
   reports 1,024 samples of PDC, worker priority Applied, zero due-boundary
   misses, zero dry fallback, and no timing warning during representative load.

3. Listen specifically for low-frequency continuity, 86.13/172.27 Hz buzz,
...
4. Submit the four blind votes from the sealed C91-vs-C193 pack. Do not reveal
   its answer key before the votes are locked. Promotion requires the user's
   explicit approval after both machine and listening evidence are reviewed.
EOF
...
printf '%s\n' \
    "C91_MAC_QUALIFICATION: MACHINE PASS" \
