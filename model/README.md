...
The complete [Linux Release rebuild and test suite after the test-buffer repair](validation/leader-cleanup-linux-plugin-validation-v2.json) pass: **162 checks**, one expected Windows-only skip and seven opt-in tests disabled. Both PyTorch parity tests, all 99 metadata fields, reset/EOF, routing, variable callback and callback heap-traffic checks pass. Project C++ sources compile as C++20 with warnings as errors. The two initial configuration failures are retained; a fresh cache with the existing pinned Linux SDK resolves them without production-code changes.

The first candidate macOS CI build, bundle verification and 162 applicable checks pass. Windows builds successfully but one test process crashes after its assertions pass. The [retained failure and sanitizer reproduction](validation/leader-cleanup-transient-test-repair.json) identify an inherited test-buffer overflow: sample index 233 in a 128-sample channel. The corrected test uses bounds-checked index 105, preserves its assertions and passes the complete Linux rerun above. The earlier [Linux result](validation/leader-cleanup-linux-plugin-validation.json) remains historical evidence from before this repair. Fresh macOS/Windows CI results are tracked in the candidate pull request.
