...
StemgenRT is a real-time music source separation plugin built with JUCE and ONNX Runtime. This tree carries the frozen c91 graph in an explicitly unqualified same-callback 512-PDC listening path and exposes four stereo stems: drums, bass, other, and vocals.
...
# Configure and build the c91 same-callback listening candidate (debug)
...
- **Audio thread**: Preserves native input for Main/fallback, submits one complete 512-sample request, waits only until the absolute 10 ms audition deadline, schedules an on-time result, and renders the current output range.
- **Inference thread**: Runs at elevated priority, owns previous-audio, overlap-add, and fusion-GRU state, and publishes through the bounded lock-free queue.
...
Initialize all persistent state tensors to zero. `separated_chunk` is aligned to the preceding call's `audio_chunk`. Sequence zero after reset is successful pre-roll with no valid output; sequence one emits sequence zero. One final zero `audio_chunk` flushes the last real hop. Defer a play-to-stop reset until after that exact 512-sample flush callback renders; shorter stop callbacks remain outside the listening contract.
...
The graph clock and hop remain fixed at 44.1 kHz and 512 samples. The same-callback listening build accepts exactly a 44.1 kHz / 512-sample prepared host configuration, where both bridge directions are exact zero-delay bypasses. The existing higher-rate bridge is preserved for later requalification but is not enabled by this listening contract.
...
The c91 graph has one hop of output delay. At callback N, the audio thread submits input N and must receive the graph's output for N-1 before that same callback renders. The queue contributes zero delay hops, so the host reports exactly 512 samples (11.61 ms) of PDC.

The real-time wait is bounded by an absolute deadline 10.0 ms after `processBlock` entry, leaving about 1.61 ms for publication, output writing, and host return. This is a provisional audition budget, not a hard-real-time guarantee. Only an exact 44.1 kHz / 512-sample callback may use model output.
...
Tag every processed hop with its exact output sample range. If inference is unavailable or misses the same-callback deadline, leave the worker and graph state continuous, discard that result when its original range has elapsed, and use the complete latency-aligned dry fallback for those samples. Never shift a late result to the current read position. The final output stage must still route the residual to `Other`, so the four enabled internal stems sum to Main throughout startup and deadline misses.
...
CPU is the reference and supported deployment path. A native c114 control on the research Ryzen measured the exact c91 ONNX at 3.84 ms mean, 6.01 ms p99, 9.10 ms p99.9, and 7 misses in 10,000 direct calls. That demonstrates feasibility but does not qualify the plugin. Promotion requires complete worker-wake/wait/drain/write timing under DAW load on each target Mac, with paired control, p99.9 margin, and miss-delta evidence.
...
Changes to the streaming path should cover, at minimum: exact graph names/shapes and model identity; OLA/past/hidden state progression; successful invalid pre-roll and one-zero-hop flush; reset determinism; sequence-gap recovery; exact 512-sample PDC; bounded wait success, timeout, epoch invalidation, and late-result discard; prepared/actual block-size mismatch and complete-Other fallback; bus ordering; finite outputs; raw-level previous-hop alignment; non-finite failure/reset; explicit low-frequency seam measurements; low-level confidence behavior; and `Main == Drums + Bass + Other + Vocals` during normal inference, startup, deadline misses, and fallback. Keep performance benchmarks stateful and measure the complete worker-wake/wait/drain/write path.
