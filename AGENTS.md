...
# Qualified release build (requires the official ONNX Runtime 1.26.0 SDK)
...
./scripts/install-plugins.sh            # qualified release build (default)
./scripts/install-plugins.sh --debug    # debug build
./scripts/install-plugins.sh --release  # qualified release build (explicit)
```
...
- **Inference thread**: Owns the model's recurrent/overlap-add state, runs ONNX inference asynchronously, and converts retained stems back to the host clock before publishing them.
...
Only the inference worker may advance model state. Reset `past_audio`, the raw aligned-past copy, `overlap_add_buffer`, `fusion_hidden`, and the model-input gain on transport starts, seeks, scrubs, loop wraps, inference failures, and input-sequence gaps. Epoch changes during an in-flight run invalidate that output; zero the state before accepting another sequence. A reset always creates a new pre-roll result.

### Host Sample-Rate Bridge

The graph clock and hop remain fixed at 44.1 kHz and 512 samples. The qualified host rates are 44.1, 48, 88.2, 96, 176.4, and 192 kHz. At 44.1 kHz both bridge directions are exact zero-delay bypasses. Reject every other host rate; never clock the graph directly at the host rate.
...
The c91 graph is materially level-sensitive, and a -10 dB copy can separate differently from the same content at ordinary level. On the inference worker, calculate one stereo-linked RMS and peak over the exact raw analysis window: current 512-sample hop on pre-roll, then raw past plus current hops. Apply a boost-only gain equal to the minimum of the gain toward `kModelInputTargetRms` (-12 dBFS RMS), the peak headroom below `kModelInputPeakCeiling` (0 dBFS), and `kModelInputMaxBoost` (+40 dB). Clamp the result to a minimum of one, so input already at or above the RMS target or peak ceiling is not attenuated. Hold the previous gain for an exact-zero hop so finite-render flush does not move state unnecessarily.

When gain changes from `Gprev` to `G`, multiply `past_audio` and `overlap_add_buffer` by `G / Gprev`, multiply the raw current hop by `G`, and do not scale `fusion_hidden` because it is nonlinear feature state. Keep a separate raw past hop for Main alignment. Divide the emitted separated tensor by `G`, preserve drums/bass/vocals, and calculate Other from the raw aligned mixture. This keeps every amplitude-domain tensor coherent without changing Main, fallback level, latency, or the graph interface. Non-finite input or gain state must fail closed and reset all streaming state.
...
The native pipeline has a two-hop minimum latency:

- 512 samples to collect and queue the current request.
- 512 samples because the graph emits the preceding hop.

Report at least 1,024 samples (23.22 ms at 44.1 kHz) to the host for PDC. The worker needs a complete 512-sample compute interval after the callback that supplies the last sample of a model hop. Because processed results are consumed only at callback boundaries, add `ceil(512 / hostBlockSize) * hostBlockSize - gcd(hostBlockSize, 512)` samples for the prepared host block size. Thus 64-, 256-, 512-, and 1,024-sample callbacks report 1,472, 1,280, 1,024, and 1,536 samples respectively. Keep Main and fallback aligned to the exact reported timeline. Do not advertise the 11.61 ms hop interval as end-to-end plugin latency.
...
CPU is the reference and supported deployment path. The worker must complete each graph run plus retained-stem output conversion inside the 11.61 ms hop interval; the host-visible PDC does not increase the recurring compute budget. Include audio-thread input conversion in complete-path benchmarks as well. A 500-hop native Windows ORT 1.26.0 qualification measured 3.69 ms mean, 4.10 ms p95, 4.42 ms p99, 5.83 ms maximum, and zero deadline misses on the development CPU. Retain underrun telemetry and benchmark the complete native path at every qualified rate on every target platform.
